...
<!-- Updated per review: New top-level Security Model section (CRITICAL-1/2/3/4, HIGH-1/2/3) -->
## 2. Security Model

### 2.1 Threat Model

mikeclaw wraps a general-purpose code agent (Claude Code with filesystem and bash access) and exposes it through multiple channels, including an HTTP API. The primary threat is that **untrusted input reaches an agent with host-level privileges**. Every design decision below flows from this reality.

**Attacker profiles**:

| Attacker | Access | Goal |
|----------|--------|------|
| Anonymous web user | `POST /api/chat` (if web channel is network-exposed) | RCE, data exfiltration, DoS |
| Authenticated web user | Same, past API key check | Session hijacking, privilege escalation, prompt injection |
| Malicious cron content | Poisoned journal/learnings fed into cron job | Persistent prompt injection, memory poisoning |
| Local CLI user | stdin on the host machine | Assumed trusted (same privilege as running the process) |
...
**Key boundary**: The web channel is the only untrusted entry point. CLI users are trusted (they already have shell access). Cron jobs are system-initiated but their content may include data from untrusted sources (journals containing user messages).

### 2.3 Trust Domains

<!-- Updated per review: Separate trust domains per security review recommendation -->

The system operates in three trust domains with different privilege levels:

| Domain | Components | Tool access | File write access | Auth required |
|--------|-----------|-------------|-------------------|---------------|
| **User-facing (web)** | Web channel sessions | Restricted via `--allowed-tools` (no `Bash` by default) | No direct write to `memory/`, `soul/`, `cron/`, `skills/` | Yes (API key) |
| **User-facing (CLI)** | CLI channel sessions | Full (local user is trusted) | Full | No (local) |
| **System** | Cron jobs, learning reflection, consolidation | Configurable per job | Consolidation cron can write `MEMORY.md`; learner appends to `learnings.md` | N/A (internal) |

### 2.4 Channel-Specific Security Profiles
...
interface ChannelSecurityProfile {
  ...
  /** Maximum claude -p timeout in milliseconds */
  maxTimeoutMs: number;
  /** Whether authentication is required */
  requireAuth: boolean;
  /** Whether post-turn learning extraction runs for this channel */
...
Prompt injection will occur. The architecture assumes this and designs for containment across six layers:

| Layer | Control | What it prevents |
|-------|---------|------------------|
| **1. Input validation** | Message length limits, character allowlists on userId/agentId, reject known injection patterns | Oversized payloads, path traversal, trivial injections |
| **2. Tool restrictions** | `--allowed-tools` per channel security profile | Blast radius: even if injected, agent cannot execute bash or write files on web channel |
| **3. Permission mode** | `--permission-mode plan` for web channel | Claude Code prompts for confirmation on sensitive operations |
| **4. Memory isolation** | Separate trust for system-read vs agent-written files (see Section 9) | Prevents persistent prompt injection via learning poisoning |
| **5. Output sanitization** | Strip sensitive patterns (API keys, private keys) from responses before returning to web clients | Data exfiltration via response content |
| **6. Audit logging** | Structured logs of all interactions, tool invocations, file modifications (see Section 15) | Detection and forensic investigation |

### 2.6 Authentication (Web Channel)

<!-- Updated per review: CRITICAL-1, CRITICAL-2 mitigations -->

The web channel requires authentication. Two modes are supported:

**Mode 1: API Key (default for MVP)**
...
**Mode 2: Session Cookies (recommended for browser UI)**

On first authenticated request (via API key or future OAuth), the server issues an `HttpOnly`, `Secure`, `SameSite=Strict` cookie containing a cryptographically random session token:
...
// Set-Cookie: mikeclaw_session=<token>; HttpOnly; Secure; SameSite=Strict; Path=/api
```

The session token maps to an internal userId on the server side. **The client never controls the userId directly.**

### 2.7 Server-Side User Identity

<!-- Updated per review: CRITICAL-2 — userId must be server-generated -->

**The userId is always server-generated, never client-controlled.** This is a critical departure from the v1.0 design.

- **API key mode**: Each API key maps to a userId in the server config. The userId is resolved from the API key, not from the request body.
- **Session cookie mode**: The userId is generated server-side on first authentication and stored in the session token map. The browser stores only the opaque session cookie.
- **CLI channel**: userId is always `"local"` (single trusted user).

The `userId` field is removed from the `POST /api/chat` request body. Any client-supplied userId is ignored.

### 2.8 Input Validation

<!-- Updated per review: HIGH-2, HIGH-4 mitigations -->

All external input is validated before reaching the orchestrator:

```typescript
/** Validates and sanitizes a string identifier (userId, agentId, sessionId).
 *  Allows only alphanumeric, hyphens, underscores, and periods.
...
function validateId(value: string, fieldName: string): string {
  if (!ID_PATTERN.test(value)) {
    throw new ValidationError(
      `Invalid ${fieldName}: must match ${ID_PATTERN}`,
      "INVALID_INPUT",
    );
  }
  return value;
...
/** Validates agentId against registered agents. */
function validateAgentId(agentId: string, registry: AgentRegistry): string {
...
function validateMessage(message: string, maxLength: number): string {
  if (typeof message !== "string" || message.length === 0) {
    throw new ValidationError("Message is required", "INVALID_INPUT");
  }
...
**Path traversal prevention**: Any component that constructs file paths from identifiers must use `path.resolve()` and verify the result starts with the expected base directory:
...
function safePath(base: string, ...segments: string[]): string {
  ...
  if (!resolved.startsWith(path.resolve(base) + path.sep) && resolved !== path.resolve(base)) {
    throw new ValidationError(
      `Path traversal detected: ${segments.join("/")}`,
      "PATH_TRAVERSAL",
    );
  }
  return resolved;
...
### 2.9 Process Pool and Concurrency Limits

<!-- Updated per review: HIGH-1 DoS prevention -->

A `ProcessPool` controls the number of concurrent `claude -p` subprocesses:
...
class ProcessPool {
  ...
  async acquire(): Promise<void> {
    ...
    if (this.queue.length >= this.maxQueueDepth) {
      throw new MikeClawError("Service at capacity", "QUEUE_OVERFLOW");
    }
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        const idx = this.queue.findIndex((e) => e.resolve === resolve);
        if (idx >= 0) this.queue.splice(idx, 1);
        reject(new MikeClawError("Queue timeout", "QUEUE_TIMEOUT"));
      }, this.queueTimeoutMs);
      this.queue.push({
...
When the queue overflows, web channel returns HTTP 503 `{ "error": "Service at capacity", "code": "QUEUE_OVERFLOW" }`.

### 2.10 Environment Sanitization for Subprocesses

<!-- Updated per review: CRITICAL-4 mitigation -->

`claude -p` subprocesses must not inherit sensitive environment variables. The `ClaudeRunner` constructs a sanitized environment:

```typescript
function sanitizedEnv(): NodeJS.ProcessEnv {
  const env = { ...process.env };
  // Keep only what claude needs
  const ALLOWED_ENV_PREFIXES = [
    "ANTHROPIC_", "CLAUDE_", "PATH", "HOME", "USER", "LANG", "LC_",
    "TERM", "NODE_", "XDG_",
  ];
  const BLOCKED_ENV = [
  ...
  for (const key of Object.keys(env)) {
    ...
    // Keep if it matches an allowed prefix
    if (!ALLOWED_ENV_PREFIXES.some((p) => key.startsWith(p))) {
      delete env[key];
    }
  }
  return env;
...
This appendix summarizes security controls. The full security model is in Section 2.

### Shell Injection Prevention

The `ClaudeRunner` uses `spawn` with an argument array, not `exec` with string interpolation. User messages are passed as a single positional argument to `claude`, never interpolated into a shell command.

```typescript
// SAFE: arguments are separate array elements
spawn("claude", ["-p", "--output-format", "json", userMessage]);

// UNSAFE (never done): string concatenation
exec(`claude -p --output-format json "${userMessage}"`);
```

### File System Access

Claude Code, when invoked via `claude -p`, runs with the full privileges of the mikeclaw process owner. The security controls are:

1. `--allowed-tools` restricts which tools are available (per channel security profile, Section 2.4)
2. `--permission-mode` controls auto-approval of sensitive operations
3. Environment sanitization prevents leaking secrets (Section 2.10)
4. File permissions (0600) on `memory/`, `sessions/`, `cron/` directories

**Production**: Run mikeclaw under a dedicated service account. Consider container isolation.

### Session Isolation

Each (channel, user, agent) tuple maps to a separate Claude Code session. User identity is server-controlled (Section 2.7). Session IDs are validated against an allowlist pattern (Section 2.8).

### Secrets in Configuration

`mikeclaw.json` may contain references to environment variables (e.g., `${MIKECLAW_WEB_API_KEY}`). The config loader resolves these at startup. The raw config file should not contain actual secrets. Secrets are never logged.
